...
export function GoogleSignInButton({ onIdToken, disabled }: GoogleSignInButtonProps) {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (isMobile) {
      // MOBILE: Force the full Google account chooser using the standard OAuth flow
      const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID
      const redirectUri = window.location.origin
      
      // Add 'prompt=select_account' to force Google to show the account picker
      ...
      if (!initialized.current && (window as any).google) {
        ;(window as any).google.accounts.id.initialize({
          ...
          auto_select: false, // <--- Force the popup to show the list, not auto-login
        })
